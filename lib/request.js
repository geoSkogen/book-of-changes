class Request {
  ...
  */
  getHexagramsLinesConfig(hex_bin_arr) {
    ...
    if (!this.#query.id) {
      console.log('Request::getHexagramsByLinesConfig requires an ID string.')
      return null
    }
    ...
    if (this.#query.moving_lines) {
      still_lines_config = this.tranformLineConfig(lines_config[0],this.#query.moving_lines)
  ...
  */
  getHexagramFilter() {
    let resposnse_obj = null
    if (this.#query.id && this.#query.segment) {
      trigram_id = ( /^[10]{3}$/.test(this.#query.id) ) ?  this.#query.id : Number(this.#query.id)
      segement = ( ['lower','upper'].indexOf(this.#query.segment) > -1 ) ? this.#query.segment : ''
      response_obj = {
        value : trigram_id,
        action : segment
      }
