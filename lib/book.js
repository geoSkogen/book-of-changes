class Book {
  ...
  */
  loadTrigram(id) {
    if (this.#trigramLineConfigurations[id]) {
      return this.buildTrigram(this.#trigramLineConfigurations[id])
    } else if (this.#trigramLineConfigurations.indexOf(id)>=0) {
      return this.buildTrigram(id)
    } else {
      console.log('Book::getTrigram was passed an unrecongized id argument.')
      return null
    }
  }
  ...
  */
  getHexagramsByTrigram(trigram_id,trigram_half_int) {
    ...
    if (typeof trigram_id === 'number') {
      ...
      return result
      console.log('Book::getHexagramsByTrigram expects a number or a string consisting only of 1 and 0 as argument 1.')
    }
