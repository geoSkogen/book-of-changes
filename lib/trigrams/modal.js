class Modal {
  ...
  */
  show(hexagram) {
    ...
    if (hexagram instanceof Hexagram) {
      ...
      this.#lowerTrigramLink.href = this.getHexagramsFilter(hexagram.trigrams[0],'bottom')

      this.#upperTrigramName.innerText = this.getTrigramTitle(hexagram.trigrams[1])

      this.#upperTrigramLink.href = this.getHexagramsFilter(hexagram.trigrams[1],'top')
  ...
  */
  getHexagramsFilter(trigram,hex_segment_str) {
    ...
    let result = ''
    if (trigram instanceof Trigram) {
      if (['bottom','top'].indexOf(hex_segment_str)>=0) {
