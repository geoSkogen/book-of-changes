...
class Builder {
  ...
  setLineArr(line_arr,line_set_index) {
    ...
    if (valid_arr) {
      if (line_set_index === 0 || line_set_index === 1) {
        ...
        // TODO: this should unset the hexagram at line_set_index -- ensure adding this won't introcude a bug.
  ...
  */
  getTrigramsLineConfig(hex_index_int) {
    ...
    if (hex_index_int === 0 || hex_index_int === 1) {
      // BUG This assumes the lines have been set by Trigram objects but not mutated by other methods later
      // TODO The trigram line configuration should be generated by concatenating the line configurations of the set Trigram objects
    ...
    if (trigram instanceof Trigram) {
      ...
      if (trigram.lines.length) {
        ...
        if (!this.#selectedTrigrams[hex_index][hex_segment]) {
          ...
          // TODO this should catch first -- otherwise the lines will be reset but the Trigram will not.
  ...
  */
  unsetLinesByTrigramIndex(hex_segment,hex_index) {
    ...
    // BUG This assumes the Trigram has been set and the Builder's current line state is still reflective of it
    // TODO Check for an existing Trigram and match its line configuration to the specified hexagram segment before un-setting the lines
