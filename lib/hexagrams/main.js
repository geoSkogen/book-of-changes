        ...
        function _init_render(hexagrams) {
          if (Array.isArray(hexagrams)) {
            ...
            for (let i = 0; i < VIEW.hexagrams.length; i++) {
              ...
              /*
              // This event listener is superfluous.
              // Because the hexagram panels are links, not buttons, the Enter key event already duplicates the click event.
              hex_element.addEventListener('keydown', function (event) {
                if (event.code === 'Enter') {
                  _init_select(hex_element)
                }
              })
