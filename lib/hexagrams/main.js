          ...
          if (Array.isArray(hexagrams)) {
            ...
            for (let i = 0; i < VIEW.hexagrams.length; i++) {
              ...
              hex_element.addEventListener('click', function (event) {
                ...
                BUILDER.toggleHexagram(
                  hexagram,
                  VIEW.toggleHexagram(hexagram.number)
                )
                MODAL.show(BUILDER.hexagrams)
              })
              hex_element.addEventListener('keyup', function (event) {
                if (event.code === 'Enter') {
                  MODAL.show([HEXAGRAMS[_get_hex_element_index(hex_element)]])
                }
              })
            }
          } else {
