    ...
    )
    {
      ...
      if (library && typeface) {
        ...
        function _init_line() {
          ...
          if (line_data) {
            VIEW.addLine(line_data.lineVals[0],0,line_data.lineIndex)
            if (line_data.lineVals[1]) {
              ...
              VIEW.addLine(line_data.lineVals[1],1,line_data.lineIndex)
            }
        ...
        MODAL.tabs['inner'].addEventListener('click', function (event) {
          MODAL.toggleTab('inner')
        })

        MODAL.tabs['inner'].addEventListener('keyup', function (event) {
          if (event.code==='Enter') {
            MODAL.toggleTab('inner')
          }
        })

        MODAL.tabs['outer'].addEventListener('click', function (event) {
          MODAL.toggleTab('outer')
        })

        MODAL.tabs['outer'].addEventListener('keyup', function (event) {
          if (event.code==='Enter') {
            MODAL.toggleTab('outer')
          }
        })
