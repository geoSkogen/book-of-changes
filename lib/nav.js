class Nav {
  ...
  constructor(config_obj) {
    ...
    if (typeof config_obj === 'object') {
      ...
      if (config_obj.competingModalElementIds && Array.isArray(config_obj.competingModalElementIds)) {
        config_obj.competingModalElementIds.forEach( (dom_id) => {
      ...
      if (config_obj.clashingModalElementIds && Array.isArray(config_obj.clashingModalElementIds)) {
        config_obj.clashingModalElementIds.forEach( (dom_id) => {
          let modal_el = document.getElementById(dom_id)
          if (modal_el) {
            SELF.#clashingModalElements.push(modal_el)
          }
        })
      }
    }
  ...
  show() {
    ...
    this.#clashingModalElements.forEach( (modal_el) => {
      if (!JSON.parse(modal_el.getAttribute('aria-hidden'))) {
