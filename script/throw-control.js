...
*
* Exports DOM objects: hex_names, hex_modal, hex_modal_closer, hex_refresh_icon, modal_dom, hexagrams_menu, hexagram_menu_tabs
...
const throw_control = {
  ...
  reset_hexes : function () {
    var hexes = [hex_builder.graph,hex_builder.regraph]
    var hex_bins = []
...
throw_control.hex_names.forEach( function (hex_name_el) {
  ...
  hex_name_el.addEventListener('click', function (event) {
    if (hex_builder.this_hex.length===6) {
      var collection = throw_control.get_hex_collection(event)
